...
<body>
		...
		<form name="" action="" method="post">
			<section id="section">
				...
				<c:forEach items="${map.goods}" var="g">
					<div class="lf box" id="d1" style="width:230px;height:250px;">
						<div class="info">
						<div class="img pic">
							<img style="cursor:pointer;" src="${basePath}${g.image}" alt="图片无法显示" title="${g.title}" onclick="toItemInfo(${g.id})" />
						</div>			
						<div class="describe">
							<p style="height:40px;overflow: hidden;cursor:pointer" onclick="toItemInfo(${g.id})">${g.title}</p>
	...
	<!--分页-->
	<div  style="text-align:center;margin:auto;width:80%;height:30px;">
		...
		<c:forEach begin="1" end="${map.pageCount}" var="p">
			...
			<c:if test="${p!=map.pageCurrent}">
				<a href="${basePath}/good/getpage.do?pageCurrent=${p}" style="color:#ccc">${p}</a>
			</c:if>
		</c:forEach>
		
		<a href="${basePath}/good/nextpage.do?pageCurrent=${map.pageCurrent}">下一页</a>
		<span style="color:green">总页数：${map.pageCount}</span>
	</div>
